...
add_filter( 'scriptlesssocialsharing_can_do_buttons', 'ch_add_buttons_archives' );
/**
 * Enable Scriptless Social Sharing buttons to be shown on archives.
 *
 * @param bool $cando Whether or not the buttons should be shown.
 * @return bool
 */
function ch_add_buttons_archives( $cando ) {
...
add_action( 'genesis_entry_content', 'ch_delete_post_link', 8 );
/**
 * Add a link to delete posts.
 *
 * @return void
 */
function ch_delete_post_link() {
	global $current_post, $current_user, $post;
	if ( is_page_template( 'page_archive.php' ) ) {
		return;
	}
	if ( $post->post_author == $current_user->ID || current_user_can( 'administrator' ) ) { ?>
   <p><a onclick="return confirm('Are you SURE you want to delete this post?')" href="<?php echo get_delete_post_link( $post->ID ) ?>">Delete Entry</a></p>
<?php }
}
...
add_action( 'genesis_after_header', 'ch_featured_image_fallback' );
/**
 * Add a fallback image to the featured image
 */
function ch_featured_image_fallback() {
	...
	if ( has_post_thumbnail() ) {
		...
		$image = 'http://placeholdit.imgix.net/~text?txtsize=60&txt=Default%20Image&w=300&h=300';
	}
	...
	printf( '<div class="singular-image"><a href="%s" rel="bookmark"><img src="%s" alt="%s" /></a></div>', get_permalink(), $image, $alt );
}
